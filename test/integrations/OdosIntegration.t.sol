...
contract OdosIntegrationTest is Test, MerkleTreeHelper {
    ...
    function testOdosSwapNoETH() external {
        deal(getAddress(sourceChain, "WETH"), address(boringVault), 1_000e18);
        deal(getAddress(sourceChain, "USDC"), address(boringVault), 1_000_000e18);
        
        //TODO check for native leaves and make sure we can send ETH with call 
        
        address inputToken= getAddress(sourceChain, "USDC");   
        address outputToken = getAddress(sourceChain, "WETH");   
       
        ManageLeaf[] memory leafs = new ManageLeaf[](4);
        _addOdosSwapLeafs(leafs, inputToken, outputToken);
        ...
        manageLeafs[2] = leafs[2]; //swap() usdc -> weth
        //manageLeafs[4] = leafs[4]; //swapCompact() usdc -> weth //TODO test swapCompact

        bytes32[][] memory manageProofs = _getProofsUsingTree(manageLeafs, manageTree);

        address[] memory targets = new address[](3);
        targets[0] = getAddress(sourceChain, "USDC"); //approve
        targets[1] = getAddress(sourceChain, "WETH"); //approve
