...
contract BalancerV3IntegrationTest is BaseTestIntegration {
    ...
    function testBalancerAddRemoveLiqSingleTokenExactIn() external {
        ...
        // get single token out
        tx_ = _getTxArrays(1); 

        tx_.manageLeafs[0] = leafs[27]; 
        ...
        tx_.targetData[0] = abi.encodeWithSignature(
            "removeLiquiditySingleTokenExactIn(address,uint256,address,uint256,bool,bytes)", getAddress(sourceChain, "balancerV3_USDC_GHO_USDT"), lpBalance, waETHUSDC, amounts, false, "" 
        ); 
