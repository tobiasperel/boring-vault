...
contract ElixirWithdrawIntegrationTest is Test, MerkleTreeHelper {
    ...
    function testElixirWithdrawIntegration() external {
        // Give BoringVault some sdeUSD.
        uint256 assets = 100_000e18;
        deal(getAddress(sourceChain, "sdeUSD"), address(boringVault), assets);
        ...
        targets[0] = getAddress(sourceChain, "sdeUSD");
        targets[1] = getAddress(sourceChain, "sdeUSD");

        bytes[] memory targetData = new bytes[](2);
        targetData[0] = abi.encodeWithSignature("cooldownAssets(uint256)", assets / 2);
        uint256 shares = ERC4626(getAddress(sourceChain, "sdeUSD")).previewWithdraw(assets / 2);
        ...
        EthenaSusde sdeUSD = EthenaSusde(getAddress(sourceChain, "sdeUSD"));
        (uint104 end, uint152 amount) = sdeUSD.cooldowns(address(boringVault));
        ...
        assertEq(
            getERC20(sourceChain, "deUSD").balanceOf(address(boringVault)),
            amount,
            "BoringVault should have received unstaked deUSD."
        );
    }
